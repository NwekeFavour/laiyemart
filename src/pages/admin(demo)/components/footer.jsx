...
const Footer = ({ storeName, storeLogo, storeDescription, storeId, storeSlug, isStarter }) => {
  ...
  const categories = useMemo(() => {
    ...
    let displayCats = uniqueNames;
    if (displayCats.length > 4) {
      displayCats = displayCats.sort(() => 0.5 - Math.random()).slice(0, 4);
    }
    return ["All", ...displayCats];
  }, [products]);

  // Helper to match your getStorePath behavior
  // Helper to build the correct URL based on plan type
const getStorePath = (path) => {
  // If it's a starter plan and we have a slug, prepend it. 
  // Otherwise, just return the path (for custom domains/subdomains).
  const cleanPath = path.startsWith('/') ? path : `/${path}`;
  return isStarter && storeSlug ? `/${storeSlug}${cleanPath}` : cleanPath;
};
  ...
  return (
    <footer className="bg-gray-900 text-gray-200 py-12">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div>
          <Avatar src={storeLogo} sx={{ width: 45, height: 45, mb: 2 }} />
          <p className="text-gray-400 text-sm italic">
            {storeDescription || `Elevating your shopping experience at ${storeName}.`}
          </p>
        </div>
