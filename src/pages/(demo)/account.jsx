...
import { Box, Typography, Card, Grid, List, ListItem, ListItemButton, ListItemDecorator, Divider, Avatar, Badge } from "@mui/joy";
import { User, Package, Heart, MapPin, LogOut, ChevronRight, CreditCard, ShoppingBag, Bell } from "lucide-react";
...
export default function CustomerAccountPage({ storeData, customer, isDark }) {
  ...
  const colors = {
    ...
    primary: "#ef4444", // Jumia/Layemart Red
            ...
            <Grid xs={12} md={3.5}>
              <Card sx={{ p: 0, bgcolor: colors.card, border: "1px solid", borderColor: colors.border, borderRadius: "sm", boxShadow: "sm" }}>
                <Box sx={{ p: 3, display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <Box>
                    <Typography level="title-md">{customer?.name || "Customer"}</Typography>
                    <Typography level="body-xs" sx={{ color: colors.textMuted }}>{customer?.email}</Typography>
                  </Box>
                </Box>
                ...
                <List sx={{ "--ListItem-radius": "0px" }}>
                  {menuItems.map((item) => (
                    <ListItem key={item.id}>
                      <ListItemButton 
                        ...
                        sx={{ 
                          py: 1.5,
                          "&.Mui-selected": { color: "slate.100", bgcolor: isDark ? "#1e293b" : "neutral.100" } 
                        }}
                      >
                        <ListItemDecorator sx={{ color: activeTab === item.id ? "slate.100" : "inherit" }}>
                          {item.icon}
                        </ListItemDecorator>
                        <Typography level="title-sm" sx={{ flex: 1, color: "inherit" }}>{item.label}</Typography>
                        {item.count && (
                          <Badge size="sm" badgeContent={item.count} color="danger" variant="solid" sx={{ mr: 2 }} />
                        )}
                  ...
                  <ListItem>
                    <ListItemButton sx={{ color: colors.primary }}>
